namespace Simple.Rest.Tests.Controllers
{
    ...
    public class EmployeesController : ApiController
    {
        ...
        public HttpResponseMessage Get(int id)
        {
            ...
            if (testHeader.Key != null)
            {
                response.Headers.Add("TestHeader", testHeader.Value);
            }

            if (testCookie != null)
            {
                response.Headers.AddCookies(new[] { new CookieHeaderValue("TestCookie", testCookie["TestCookie"].Value) });
            }
